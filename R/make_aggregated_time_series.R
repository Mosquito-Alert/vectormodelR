...
make_aggregated_time_series = function(x, start=NA, end=NA, interval="day", groups = c("trap_ID", "TigacellID")){
  ...
  result = x %>%
    dplyr::group_by_at(groups_and_date) %>%
    dplyr::summarise(count = dplyr::n()) %>%
    dplyr::ungroup() %>%
    dplyr::group_by_at(groups) %>%
    tidyr::complete(date = seq.Date(this_start, this_end, by = interval), fill = list(count = 0)) %>%
    dplyr::ungroup() %>%
    dplyr::filter(date >= this_start, date <= this_end) %>%
