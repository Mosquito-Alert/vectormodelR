...
#' given location and applies one or more enrichment helpers (weather, landcover,
#' NDVI, elevation, population density, pseudoabsences) in the order supplied.
...
#'   feature steps to run. Accepted values: `"wx"`/`"weather"`,
...
add_features <- function(
  ...
  if (any(is.na(canonical))) {
    ...
    stop(
      ...
      ". Allowed values include wx, lc, ndvi, el, pd, se.",
      call. = FALSE
    )
  }
