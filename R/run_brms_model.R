...
#'
#' Fits a Bayesian GLM/GAM on the Mosquito Alert occupancy dataset using `brms`.
#' This model does NOT include the spatial BYM2 random effect. It uses the same
...
#' @param threads_per_chain Integer. Number of threads per chain for within-chain parallelism.
#' @param adapt_delta Numeric. Target average proposal acceptance probability. Default 0.995.
#' @param max_treedepth Integer. Max tree depth for NUTS. Default 20.
#' @param backend Character. "cmdstanr" (default) or "rstan".
#' @param iso3,admin_level,admin_name Optional strings to locate the dataset if `dataset` is NULL.
#' @param write_output Logical. Whether to save the fitted model to disk.
#' @param output_path,input_dir Paths for output/input.
#' @param save_pars Logical. Forwarded to `brms::brm`.
#' @param verbose Logical.
#'
#' @return The fitted `brmsfit` object.
#' @export
run_brms_model <- function(
  ...
  if (is.null(priors)) {
    ...
    if (isTRUE(verbose)) {
      message("No priors supplied; using default standard priors.")
    }
  }
  ...
  if (isTRUE(verbose)) {
    message("Fitting standard brms model with ", nchains, " chain(s) using backend '", backend, "'.")
  ...
  if (isTRUE(write_output)) {
    ...
    if (isTRUE(verbose)) message("Standard brms model saved to ", final_output_path)
