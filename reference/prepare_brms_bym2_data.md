...
- model_data:
  ...
  The prepared data frame ready for brms.

- adjacency:

  The aligned sparse adjacency matrix.

- grid_col:

  The active grid identifier column.

- scaling:

  Scaling parameters from
  [`prepare_model_data()`](https://labs.mosquitoalert.com/mosquitoR/reference/prepare_model_data.md).

- scale_specs:

  The normalized scaling specifications.
...
- factor_cols:
  ...
  The columns converted to factors.

- meta:

  Preparation and adjacency metadata.
...
Use
[`prepare_model_data()`](https://labs.mosquitoalert.com/mosquitoR/reference/prepare_model_data.md)
for non-spatial models and `prepare_brms_bym2_data()` when the model
requires a BYM2/CAR spatial component.
